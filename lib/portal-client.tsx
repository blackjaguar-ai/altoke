...
export function useSala(roomId: string, handle: string, district: string, rolInicial: RolSala = "bidder") {
  ...
  const { messages, send, presence, typing, sendTyping, activity, sendActivity, setMetadata, me, status } =
    ...
    if (presence?.kind === "detailed") {
      ...
      return presence.participants
        ...
        .map((p: any) => ({
          ...
          // MI PROPIA fila usa `rol` (estado local, cambia al instante con
          // el click) en vez de p.metadata?.role. Bug real visto en vivo:
          // al pasar a espectador, "N mirando" seguía en 0 - setMetadata
          // manda el cambio al servidor y ESTE vuelve a anunciarlo por
          // delta, pero no hay forma de verificar desde acá si ese delta
          // se reenvía también a la MISMA conexión que lo originó. En vez
          // de apostar a esa suposición, para mi propia fila confío en lo
          // que YO sé con certeza que acabo de elegir.
          role: (String(p.id) === me?.id ? rol : (p.metadata?.role as RolSala | undefined)) ?? "bidder",
        }));
    }
    return [];
  }, [presence, me?.id, rol]);
